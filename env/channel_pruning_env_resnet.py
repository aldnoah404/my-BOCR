...
class ChannelPruningEnv:
    ...
    def prune_kernel(self, op_idx, preserve_ratio, preserve_idx=None):
        ...
        if weight.shape[2] == 1:  # 1x1 conv or fc
            # rec_weight, _ = torch.linalg.lstsq(input=Y, A=masked_X)
            rec_weight = torch.linalg.lstsq(masked_X, Y).solution
            ...
            # rec_weight, _ = torch.linalg.lstsq(input=Y, A=X_fit)
            rec_weight = torch.linalg.lstsq(X_fit, Y).solution
