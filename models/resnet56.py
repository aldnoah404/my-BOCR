...
class P_BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, mid_planes, stride=1, option='A'):
...
class pruned_resnet(nn.Module):
    def __init__(self, block, num_blocks, ratios, num_classes=10):
        ...
        self.conv1 = nn.Conv2d(3, 16, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(16)
        self.layer1 = self._make_purned_layer(block, 16, num_blocks[0], ratios[1:10], stride=1)
        self.layer2 = self._make_purned_layer(block, 32, num_blocks[1], ratios[10:19], stride=2)
        self.layer3 = self._make_purned_layer(block, 64, num_blocks[2], ratios[19:28], stride=2)
        self.linear = nn.Linear(64, num_classes)
    ...
    def _make_purned_layer(self, block, planes, num_blocks, ratios, stride):
        strides = [stride] + [1]*(num_blocks-1)
        layers = []
        i = 0
        for stride in strides:
            layers.append(block(self.in_planes, planes, ratios[i], stride))
            self.in_planes = planes * block.expansion
            i+=1
